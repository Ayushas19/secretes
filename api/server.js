...
async function getTodayEventWindow() {
  ...
  // 20:30 IST in UTC
  const endIST = new Date(year, month, day, 20, 30, 0, 0);
  const endUTC = new Date(endIST.getTime() - IST_OFFSET_MS - new Date().getTimezoneOffset() * 60000);

  // 20:31 IST in UTC (for data export)
  const exportIST = new Date(year, month, day, 20, 31, 0, 0);
...
app.post("/api/test/reset-event", async (req, res) => {
  try {
    const now = Date.now();
    const windowData = {
      startMs: now + 5 * 60 * 1000,        // Starts in 5 minutes (Lobby active)
      endMs: now + 65 * 60 * 1000,         // Ends in 65 minutes
      exportMs: now + 66 * 60 * 1000,
    };
    await storage.setEventWindow(windowData);
    
    // Also clear all registered participants so we can login with the same usernames again!
    if (storage.isMongo) {
      const db = await getMongoDb();
    ...
    res.json({ ok: true, message: "Event reset to Lobby phase, and registered participants cleared." });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
    ...
    if (p) {
      ...
      return res.json({
        ...
        data: {
          ...
          proctorWarnings: p.proctorWarnings || 0,
          proctorTerminated: p.proctorTerminated || false,
          scoreDeduction: p.scoreDeduction || 0,
        },
      });
    }
...
app.post("/api/fix/update", async (req, res) => {
  try {
    ...
    if (p.proctorTerminated || p.fixLocked || (await getEventPhase()) === "ended") {
      return res.json({ ok: true, locked: true }); // already locked or terminated
    }
...
app.post("/api/explanation/update", async (req, res) => {
  try {
    ...
    if (!p) return res.status(404).json({ ok: false, error: "Participant not found." });
    if (p.proctorTerminated || p.finalSubmitted || (await getEventPhase()) === "ended") {
      return res.json({ ok: true, locked: true });
    }
...
// 7b. Proctoring: Violation Logging / Warning Incrementor
app.post("/api/proctor/violation", async (req, res) => {
  try {
    ...
    if (!p) return res.status(404).json({ ok: false, error: "Participant not found." });
    if (p.proctorTerminated || p.finalSubmitted || (await getEventPhase()) === "ended") {
      return res.json({ ok: false, error: "Proctoring session completed or exam ended." });
    }

    p.proctorWarnings = (p.proctorWarnings || 0) + 1;
    p.proctorLogs = p.proctorLogs || [];
    p.proctorLogs.push({
      timestamp: Date.now(),
      type: type || "UNKNOWN",
    ...
    res.json({
      ok: true,
      warnings: p.proctorWarnings,
      terminated: p.proctorTerminated,
...
// 7c. Proctoring: Record Completed Room Audit
app.post("/api/proctor/audit", async (req, res) => {
  try {
    const { username, auditMinute } = req.body;
    const p = await storage.getParticipant(username);

    if (!p) return res.status(404).json({ ok: false, error: "Participant not found." });
    
    p.completedAudits = p.completedAudits || [];
    if (!p.completedAudits.includes(auditMinute)) {
      p.completedAudits.push(auditMinute);
    }
    
    await storage.setParticipant(username, p);
    res.json({ ok: true, completedAudits: p.completedAudits });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
  ...
  try {
    ...
    const formatted = all.map((p) => {
      ...
      return {
        ...
        proctorTerminated: p.proctorTerminated || false,
        scoreDeduction: p.scoreDeduction || 0,
        proctorLogs: p.proctorLogs || [],
        completedAudits: p.completedAudits || [],
      };
    });
